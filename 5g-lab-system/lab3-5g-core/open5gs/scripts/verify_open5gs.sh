...
if systemctl list-unit-files | grep -q '^open5gs-nrfd.service'; then
  echo ""
...
if systemctl list-unit-files | grep -q '^open5gs-nrfd.service' && [[ "$(systemctl is-active open5gs-nrfd 2>/dev/null)" == "active" ]]; then
  echo ""
